...
xgboost_model = load_model('xgb_model.ptk')

naive_bayes_model = load_model('gn_model.ptk')

random_forest_model = load_model('rf_model.ptk')

decision_tree_model = load_model('dt_model.ptk')

svm_model = load_model('svm_model.ptk')

knn_model = load_model('knn_model.ptk')
...
def explain_prediction(probability, input_dict, surname):
  ...
  customer_info = pd.DataFrame(list(input_dict.items()),
                               columns=["Feature", "Value"])
  customer_info_table = customer_info.to_markdown(index=False)

  churned_stats = df[df['Exited'] == 1].describe().to_markdown()
  non_churned_stats = df[df['Exited'] == 0].describe().to_markdown()
  ...
  - Your explanation should be based on the customer's information, the summary statistics, and the feature importances provided.
...
don't also write stuff like <more lines that just show the extra information>
More Products: We would like to recommend that you take advantage of additional services that we offer, such as loans and mortgages, which can provide you with a more comprehensive financial solution.

Recommendations for an improved financial stability:_ Since the person is so hard to understand how this is showing on the other bank details with this France there isn’t many available that really knows no exact know about the harder who like a wise will let us have some more wise.

This should be solely the email not recommendations for company
Don't include note points
don't do stuff like [Your name]
"""
